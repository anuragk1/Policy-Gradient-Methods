...
# Model
class VPG:
    def __init__(self, env, policy_hidden_layers=POLICY_HIDDEN_LAYERS, value_hidden_layers=VALUE_HIDDEN_LAYERS):
    ...
    def train(self, epochs=EPOCHS, episodes_per_epoch=EPISODES_PER_EPOCH, value_lr=VALUE_FN_LEARNING_RATE,
             policy_lr=POLICY_LEARNING_RATE, gamma=GAMMA, max_traj_length=MAX_TRAJ_LENGTH, RENDER=False, PLOT_REWARDS=True, VERBOSE=False):
        ...
        for epoch in range(epochs):
            ...
            value_loss = torch.tensor([[0.0]], device=device, dtype=dtype, requires_grad=True)
            policy_loss = torch.tensor([[0.0]], device=device, dtype=dtype, requires_grad=True)
            ...
            for _ in range(episodes_per_epoch):
                ...
                traj_obs_values = torch.tensor(traj_obs_values, device=device, dtype=dtype)
                value_loss = value_loss + nn.MSELoss()(disc_rewards, traj_obs_values)

                for j in range(len(traj_rewards) - 1):
                    with torch.no_grad():
                        advantage = (disc_rewards[j] - traj_obs_values[j])

                    policy_loss = (policy_loss + -1 * traj_action_distributions[j].log_prob(traj_actions[j]) * advantage)
            ...
            if VERBOSE and (epoch == 0 or ((epoch + 1) % (epochs / 10)) == 0):
                print(f"Epoch {epoch+1}: Average Episodic Reward = {avg_episode_reward:.2f} |" f" Value Loss = {value_loss.item():.2f} |" f" Policy Loss = {policy_loss.item():.2f}")
    ...
    def save(self, path=None):
        ...
        torch.save({"policy_state_dict": self.policy.state_dict(), "value_state_dict": self.value.state_dict()}, path)
